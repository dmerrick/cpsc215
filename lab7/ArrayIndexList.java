...
public class ArrayIndexList<E> implements IndexList<E> {
   ...
   */
  public void reverse() {
   // the left and rightmost elements
   int left,right;

   left = 0; // the first element
   right = size() - 1; // the last element
  
   // start swapping the elements
   while (left < right) {
      E temp = A[left]; 
      A[left] = A[right]; 
      A[right] = temp;
